...
Rust 标准库提供了多种强大的集合类型，用于存储和操作多个值。本模块将深入探讨 Vec、HashMap、BTreeMap、VecDeque 等常用集合类型的使用方法和最佳实践。
...
- 掌握 HashMap 和 BTreeMap 的使用
- 了解 HashSet 和 BTreeSet 的区别和使用场景
- 理解 VecDeque 双端队列的用途
- 理解迭代器的使用
- 掌握集合的性能特性
...
BTreeMap 和 BTreeSet 是基于有序树的集合，与 HashMap 和 HashSet 不同，它们保持键的有序性：
...
// 集合操作
let set2: BTreeSet<_> = [1, 2, 3].iter().cloned().collect();

// 并集、交集、差集
let union: BTreeSet<_> = set.union(&set2).cloned().collect();
let intersection: BTreeSet<_> = set.intersection(&set2).cloned().collect();
```
...
VecDeque 是双端队列，支持在两端高效地添加和删除元素：
...
- `btreemap.rs` - BTreeMap 和 BTreeSet 演示
- `vecdeque.rs` - VecDeque 双端队列演示
- `iterators.rs` - 迭代器演示
...
|------|----------|------|
| 索引访问 | Vec | O(1) 随机访问 |
| 键值查找（无序） | HashMap | O(1) 平均查找 |
| 键值查找（有序） | BTreeMap | O(log n) 有序查询 |
| 唯一值和快速查找 | HashSet | O(1) 去重和查找 |
| 有序唯一值 | BTreeSet | O(log n) 有序操作 |
| 双端操作 | VecDeque | O(1) 前后端操作 |
| 保持插入顺序 | Vec 或 IndexMap | 按插入顺序迭代 |
...
6. **需要有序数据时使用 BTreeMap**：而不是用 Vec 排序后查找
7. **双端操作使用 VecDeque**：而不是用 Vec 进行 push/pop(0)
8. **考虑使用 IndexMap**：需要保留插入顺序的键值映射
